...
# UCRT64
%package -n ucrt64-termcap
Summary:        MinGW terminal feature database

%description -n ucrt64-termcap
This is the GNU termcap library -- a library of C functions that
enable programs to send control strings to terminals in a way
independent of the terminal type.  The GNU termcap library does not
place an arbitrary limit on the size of termcap entries, unlike most
...
%package -n ucrt64-termcap-static
Summary:        Static version of the cross compiled termcap library
Requires:       ucrt64-termcap = %{version}-%{release}

%description -n ucrt64-termcap-static
Static version of the cross compiled termcap library.
...
make install -C build_ucrt64 \
  prefix=$RPM_BUILD_ROOT%{ucrt64_prefix} \
  exec_prefix=$RPM_BUILD_ROOT%{ucrt64_prefix} \
  oldincludedir=
...
mkdir -p $RPM_BUILD_ROOT%{ucrt64_bindir}
install -m 0755 build_win64/libtermcap-0.dll $RPM_BUILD_ROOT%{ucrt64_bindir}
install -m 0755 build_win64/libtermcap.dll.a $RPM_BUILD_ROOT%{ucrt64_libdir}
...
mkdir -p $RPM_BUILD_ROOT%{ucrt64_infodir}
mv $RPM_BUILD_ROOT%{ucrt64_prefix}/info/* $RPM_BUILD_ROOT%{ucrt64_infodir}
...
%files -n ucrt64-termcap
%doc COPYING
%{ucrt64_bindir}/libtermcap-0.dll
%{ucrt64_libdir}/libtermcap.dll.a
%{ucrt64_includedir}/termcap.h
%{ucrt64_infodir}/*

%files -n ucrt64-termcap-static
%{ucrt64_libdir}/libtermcap.a


%changelog
* Mon Aug 19 2024 Jack Greiner <jack@emoss.org> - 1.3.1-38
